                  ...
                  Container(
                    child: BlocBuilder<RoleBloc, RoleStates>(
                      builder: (context, stateRole) {
                        if (stateRole is RoleFetchingState) {
                          return Padding(
                            padding: EdgeInsets.all(8),
                            child: CircularProgressIndicator(),
                          );
                        }
                        if (stateRole is RoleEmptyState) {
                          return Padding(
                            padding: EdgeInsets.all(8),
                            child: Text('No role found !!!'),
                          );
                        }
                        if (stateRole is RoleFetchingErrorState) {
                          return Padding(
                            padding: EdgeInsets.all(8),
                            child: Text('Error Fetching Roles !!!'),
                          );
                        }
                        if (stateRole is RoleFetchedState) {
                          if (isInit == false) {
                            newRole = widget.user.role.name;
                            roleNames.clear();
                            stateRole.roles.forEach((element) {
